...
public class GUIFactionPerm extends GuiScreen {
	private final static List<TabGui> TABS = new ArrayList<TabGui>();
	private final ResourceLocation bg = new ResourceLocation("castel", "textures/gui/factionPerm.png");
	private static Minecraft mc;

	private final int xSize = 280;
	private final int ySize = 167;

	private int guiX;
	private int guiY;

	public GUIFactionPerm(Minecraft mc) {
		initTabs();
		this.mc = mc;
	}
	...
	public void initGui() {
		...
		for (i = 0; i < Math.floor((float) (TABS.size()) / 2); i++) {
			...
			buttonList.add(new GUICustomButton(i, (guiX - 26), guiY + 20 + 50 * i, "").bindTexture(bg, 512, 512)
					.setButtonDim(26 + di*2, 167, 26, 32));
		}
	...
	public void drawScreen(int mouseX, int mouseY, float ticks) {
		...
		GUICastel.drawCenteredString(GUICastel.factionName, this.width / 2, this.height / 2 - 75, 0x636363);

		super.drawScreen(mouseX, mouseY, ticks);
		int i;
		for (i = 0; i < Math.floor((float) (TABS.size()) / 2); i++) {
			GUICastel.drawScaledCustomSizeModalRect(guiX - 20, guiY + 26 + 50 * i, 18 * i, 199, 18, 18, 18, 18, 512,
					512);
		}

		for (int j = 0; j < Math.ceil((float) (TABS.size()) / 2); j++) {
			GUICastel.drawScaledCustomSizeModalRect(guiX + xSize + 2, guiY + 26 + 50 * j, 18 * (j + i), 199, 18, 18, 18,
					18, 512, 512);
		}
	}
