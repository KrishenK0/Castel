package fr.krishenk.castel.client.renderer;
...
public class RenderOrb extends Render {
	...
	private RenderBlocks itemRenderBlocks = new RenderBlocks();
	 ...
	 public void doRenderOrb(EntityOrb par1EntityProjectile, double par2, double par4, double par6, float par8, float par9) {
	      GL11.glPushMatrix();
	...
	private void renderSprite(Tessellator tessellator, IIcon icon) {
	...
	private void renderDroppedItem(ItemStack item, IIcon icon, float par4, float par5, float par6, float par7, float par8) {
		Tessellator tessellator = Tessellator.instance;
		if(icon == null) {
			...
			icon = ((TextureMap)f4.getTexture(f5)).registerIcon("missingno");
		}
		
		float f41 = icon.getMinU();
		float f51 = icon.getMaxU();
		...
		GL11.glColor4f(par5, par4, par7, 1.0F);
		ItemRenderer.renderItemIn2D(tessellator, f51, f6, f41, f7, icon.getIconWidth(), icon.getIconHeight(), f12);
		if(item != null && item.hasEffect(0)) {
			...
			GL11.glPushMatrix();
			GL11.glScalef(par8, par8, par8);
			f15 = (float)(Minecraft.getSystemTime() % 4873L) / 4873 * 8.0F;
			GL11.glTranslatef(-f15, 0.0F, 0.0F);
	...
	public void doRender(Entity entity, double par2, double par4, double par6, float par8, float par9) {
		this.doRenderOrb((EntityOrb)entity, par2, par4, par6, par8, par9);
	}
	
	protected ResourceLocation func_110779_a(EntityOrb entityOrb) {
		return entityOrb.isArrow()?field_110780_a:super.renderManager.renderEngine.getResourceLocation(entityOrb.getItemDisplay().getItemSpriteNumber());
	}
	
	protected ResourceLocation getEntityTexture(Entity entity) {
		return this.func_110779_a((EntityOrb)entity);
	}
