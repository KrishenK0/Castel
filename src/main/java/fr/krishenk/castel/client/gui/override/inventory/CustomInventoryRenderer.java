...
public abstract class CustomInventoryRenderer extends GUICustomContainer {
	private boolean field_147045_u;
	
	public CustomInventoryRenderer(Container container) {
		super(container);
	}
	...
	private void func_147044_g() {
		...
		if (!collection.isEmpty()) {
			...
			for (Iterator iterator = this.mc.thePlayer.getActivePotionEffects().iterator(); iterator
					.hasNext(); j += k) {
				PotionEffect potioneffect = (PotionEffect) iterator.next();
				Potion potion = Potion.potionTypes[potioneffect.getPotionID()];
				GL11.glColor4f(1.0F, 1.0F, 1.0F, 1.0F);
				this.mc.getTextureManager().bindTexture(bg);
				this.drawTexturedModalRect(i, j, 0, 166, 120, 32);
