...
public class EntityOrb extends EntityThrowable {
	...
	public EntityOrb(World par1World) {
		super(par1World);
		this.effect = 0;
		this.duration = 5;
		this.amplify = 0;
	...
	public EntityOrb(World par1World, EntityLivingBase par2EntityLiving, ItemStack item) {
		super(par1World);
		this.effect = 0;
		this.duration = 5;
		this.amplify = 0;
		...
		this.setSize((float) (super.dataWatcher.getWatchableObjectInt(23) / 10),
				(float) (super.dataWatcher.getWatchableObjectInt(23) / 10));
	...
	public void setThrowableHeading(double par1, double par3, double par5, float par7, float par8) {
		...
		super.motionX = (double) (MathHelper.sin(yaw / 180.0F * 3.1415927F)
				* MathHelper.cos(pitch / 180.0F * 3.1415927F));
		super.motionZ = (double) (MathHelper.cos(yaw / 180.0F * 3.1415927F)
				* MathHelper.cos(pitch / 180.0F * 3.1415927F));
	...
	public void shoot(float speed) {
		double varX = (double) (-MathHelper.sin(super.rotationYaw / 180.0F * 3.1415927F)
				* MathHelper.cos(super.rotationPitch / 180.0F * 3.1415927F));
		double varZ = (double) (MathHelper.cos(super.rotationYaw / 180.0F * 3.1415927F)
				* MathHelper.cos(super.rotationPitch / 180.0F * 3.1415927F));
	...
	public void onUpdate() {
		...
		if (super.prevRotationPitch == 0.0F && super.prevRotationYaw == 0.0F) {
			...
			super.prevRotationYaw = super.rotationYaw = (float) (Math.atan2(super.motionX, super.motionZ) * 180.0D
					/ 3.141592653589793D);
			super.prevRotationPitch = super.rotationPitch = (float) (Math.atan2(super.motionY, (double) block) * 180.0D
					/ 3.141592653589793D);
			if (this.isRotating()) {
		...
		if (this.effect == 1 && !this.field_70193_a) {
			this.setFire(1);
		}
		...
		if ((this.isArrow() || this.sticksToWalls()) && var17 != null) {
			...
			AxisAlignedBB vec3 = var17.getCollisionBoundingBoxFromPool(super.worldObj, this.xTile, this.yTile,
					this.zTile);
		...
		if (this.field_70193_a) {
			...
			Vec3 vec31 = Vec3.createVectorHelper(super.posX + super.motionX, super.posY + super.motionY,
			...
			vec31 = Vec3.createVectorHelper(super.posX + super.motionX, super.posY + super.motionY,
					super.posZ + super.motionZ);
			...
			if (!super.worldObj.isRemote) {
				...
				List f = super.worldObj.getEntitiesWithinAABBExcludingEntity(this, super.boundingBox
						.addCoord(super.motionX, super.motionY, super.motionZ).expand(1.0D, 1.0D, 1.0D));
				double f2 = 0.0D;
				...
				for (int f4 = 0; f4 < f.size(); ++f4) {
					...
					if (entity1.canBeCollidedWith()
							&& (!entity1.isEntityEqual(this.thrower) || this.field_70195_i >= 25)) {
						float f1 = 0.3F;
			...
			if (movingobjectposition != null) {
				if (movingobjectposition.typeOfHit == MovingObjectType.BLOCK
						&& super.worldObj.getBlock(movingobjectposition.blockX, movingobjectposition.blockY,
			...
			for (super.rotationPitch = (float) (Math.atan2(super.motionY, (double) var20) * 180.0D
					/ 3.141592653589793D); super.rotationPitch
							- super.prevRotationPitch < -180.0F; super.prevRotationPitch -= 360.0F) {
				;
			}
			...
			super.rotationPitch = super.prevRotationPitch + (super.rotationPitch - super.prevRotationPitch)
					+ var21 * 0.2F;
			...
			if (this.isInWater()) {
				if (super.worldObj.isRemote) {
					for (int var24 = 0; var24 < 4; ++var24) {
						float var26 = 0.25F;
						super.worldObj.spawnParticle("bubble", super.posX - super.motionX * (double) var26,
								super.posY - super.motionY * (double) var26,
								super.posZ - super.motionZ * (double) var26, super.motionX, super.motionY,
								super.motionZ);
					}
				}
			...
			if (super.worldObj.isRemote && !super.dataWatcher.getWatchableObjectString(22).equals("")) {
				super.worldObj.spawnParticle(super.dataWatcher.getWatchableObjectString(22), super.posX, super.posY,
	...
	protected void onImpact(MovingObjectPosition movingobjectposition) {
		...
		if (movingobjectposition.entityHit != null) {
			if (this.callback != null && this.callbackItem != null
					&& movingobjectposition.entityHit instanceof EntityLivingBase && this.callback.onImpact(this,
							(EntityLivingBase) movingobjectposition.entityHit, this.callbackItem)) {
				return;
			}
			...
			if (movingobjectposition.entityHit.attackEntityFrom(DamageSource.causeThrownDamage(this, this.getThrower()),
				...
				if (this.isBlock()) {
					super.worldObj.playAuxSFX(2001, (int) movingobjectposition.entityHit.posX,
							(int) movingobjectposition.entityHit.posY, (int) movingobjectposition.entityHit.posZ,
							Item.getIdFromItem(this.getItem()));
				} else if (!this.isArrow() && !this.sticksToWalls()) {
					for (var15 = 0; var15 < 8; ++var15) {
						super.worldObj.spawnParticle("iconcrack_" + Item.getIdFromItem(this.getItem()), super.posX,
								super.posY, super.posZ, super.rand.nextGaussian() * 0.15D,
								super.rand.nextGaussian() * 0.2D, super.rand.nextGaussian() * 0.15D);
					}
				}
				...
				if (this.punch > 0) {
					...
					if (var16 > 0.0F) {
						movingobjectposition.entityHit.addVelocity(
								super.motionX * (double) this.punch * 0.6000000238418579D / (double) var16, 0.1D,
								super.motionZ * (double) this.punch * 0.6000000238418579D / (double) var16);
					}
				}
				...
				if (this.effect != 0 && movingobjectposition.entityHit instanceof EntityLivingBase) {
					if (this.effect != 1) {
						...
						((EntityLivingBase) movingobjectposition.entityHit)
								.addPotionEffect(new PotionEffect(var15, this.duration * 20, this.amplify));
					} else {
			...
			if (this.isBlock()) {
				super.worldObj.playAuxSFX(2001, MathHelper.floor_double(super.posX),
						MathHelper.floor_double(super.posY), MathHelper.floor_double(super.posZ),
						Item.getIdFromItem(this.getItem()));
			} else {
				for (var12 = 0; var12 < 8; ++var12) {
					super.worldObj.spawnParticle("iconcrack_" + Item.getIdFromItem(this.getItem()), super.posX,
							super.posY, super.posZ, super.rand.nextGaussian() * 0.15D, super.rand.nextGaussian() * 0.2D,
							super.rand.nextGaussian() * 0.15D);
				}
			}
		} else {
			...
			axisalignedbb = MathHelper.sqrt_double(
					super.motionX * super.motionX + super.motionY * super.motionY + super.motionZ * super.motionZ);
		...
		if (this.explosive) {
			Iterator iterator;
			if (this.explosiveRadius == 0 && this.effect != 0) {
				if (this.effect == 1) {
					...
					super.worldObj.playAuxSFX(2002, (int) Math.round(super.posX), (int) Math.round(super.posY),
							(int) Math.round(super.posZ), this.getPotionColor(this.effect));
				}
			} else {
				boolean var13 = super.worldObj.getGameRules().getGameRuleBooleanValue("mobGriefing")
						&& this.explosiveDamage;
				Explosion var17 = new Explosion(super.worldObj, this, super.posX, super.posY, super.posZ,
						(float) this.explosiveRadius);
				var17.isFlaming = this.effect == 1;
				...
				if (!super.worldObj.isRemote) {
					...
					while (iterator.hasNext()) {
						...
						if (entitylivingbase.getDistanceSq(super.posX, super.posY, super.posZ) < 4096.0D) {
							((EntityPlayerMP) entitylivingbase).playerNetServerHandler
									.sendPacket(new S27PacketExplosion(super.posX, super.posY, super.posZ,
											(float) this.explosiveRadius, var17.affectedBlockPositions,
											(Vec3) var17.func_77277_b().get(entitylivingbase)));
						}
					}
				}
	...
	public void writeEntityToNBT(NBTTagCompound par1NBTTagCompound) {
		...
		par1NBTTagCompound.setTag("direction",
				this.newDoubleNBTList(new double[] { super.motionX, super.motionY, super.motionZ }));
		...
		par1NBTTagCompound.setInteger("PotionEffect", 0);
	...
	public void readEntityFromNBT(NBTTagCompound par1NBTTagCompound) {
		...
		this.effect = par1NBTTagCompound.getInteger("PotionEffect");
		...
		if (par1NBTTagCompound.hasKey("direction")) {
			...
			super.motionX = Double.valueOf(var2.getStringTagAt(0));
			super.motionY = Double.valueOf(var2.getStringTagAt(1));
			super.motionZ = Double.valueOf(var2.getStringTagAt(2));
		}
	...
	private int getPotionEffect(int p) {
		switch (p) {
		case 7:
			return 7;
		case 1:
